...
public abstract class ConstantsBase {
    ...
    try {
      ...
      for (int i = 0; i < lines.length; i++) {
        ...
        for (int j = 0; j < constants.size(); j++) {
          ConstantsBase.Constant constant = (ConstantsBase.Constant)constants.elementAt(j);
  ...
  public static String generateHtml() {
    String str = "<html><head><title>Constants</title></head><body>"
            + "<form method=\"post\">";
    for(int i = 0; i < constants.size(); ++i) {
      str+= ((ConstantsBase.Constant) constants.elementAt(i)).toHtml();
    }
  ...
  public static void writeConstant(String name, double value) {
    ConstantsBase.Constant constant;
    for (int i = 0; i < constants.size(); i++) {
      constant = ((ConstantsBase.Constant) constants.elementAt(i));
